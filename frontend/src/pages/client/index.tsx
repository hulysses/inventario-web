  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (response) {
        ...
        toast("Erro ao excluir cliente. O mesmo pode ter pedidos vinculados.");
      }
    } catch (error) {
      toast("Erro ao excluir cliente. O mesmo pode ter pedidos vinculados.");
    }
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 bg-background mx-7 mb-7 rounded-xl">
      ...
      <h3 className="font-light text-muted-foreground">
        Gerencie seus clientes, podendo alterar, excluir ou criar novos.
      </h3>
      <div className="flex items-center justify-end space-x-4 mt-7">
        <h1 className="font-bold text-2xl -ml-0 flex mx-auto">
          Clientes <p className="ml-5 text-gray-400">{clientQuantity}</p>
        </h1>
        {filters.map((column) => (
          <TableFilter
            key={column}
