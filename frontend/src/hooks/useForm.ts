...
export const useFormData = (
    ...
    fields.forEach((field) => {
      switch (field.name) {
        ...
        case "endereco":
          ...
          break;
        case "imagem":
          schemaObject[field.name] = z.string().refine(isValidImageUrl, {
            message: "A URL não é uma imagem válida.",
          });
          break;
        case "quantidade":
          schemaObject[field.name] = z.string().refine(validarQuantidade, {
  ...
  useEffect(() => {
    if (initialData && !isInitialDataLoaded.current) {
